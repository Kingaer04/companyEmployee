...
namespace CompanyEmployees.Utitlity
{
    public class EmployeeLinks : IEmployeeLinks
    {
        ...
        private List<ShapedEntity> ShapeData(IEnumerable<EmployeeDto> employeesDto, string fields) =>
        ...
        private LinkResponse ReturnShapedEmployees(List<ShapedEntity> shapedEmployees) =>
            new LinkResponse { ShapedEntities = shapedEmployees };

        private LinkResponse ReturnLinkedEmployees(IEnumerable<EmployeeDto> employeesDto, string fields, Guid companyId, HttpContext httpContext, List<ShapedEntity> shapedEmployees)
        {
            ...
            for (var index = 0; index < employeeDtoList.Count(); index++)
            {
                ...
                shapedEmployees[index].Entity.TryAdd("Links", employeeLinks);
            }

            var employeeCollection = new LinkCollectionWrapper<ShapedEntity>(shapedEmployees);
        ...
        private LinkCollectionWrapper<ShapedEntity> CreateLinksForEmployees(HttpContext httpContext, LinkCollectionWrapper<ShapedEntity> employeesWrapper)
        {
